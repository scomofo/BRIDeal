...
class JDAuthManager:
    ...
    def get_access_token(self) -> Optional[str]:
        """
        Retrieves the current access token. If expired, attempts to refresh it.

        Returns:
            Optional[str]: The access token, or None if unavailable or refresh fails.
        """
        if not self.is_operational:
            logger.debug("JDAuthManager: Not operational, cannot provide access token.")
            return None

        if not self.access_token:
            logger.info("JDAuthManager: No access token available. Authorization might be required.")
            return None # Or trigger auth flow if applicable

        if self.is_token_expired():
            logger.info("JDAuthManager: Access token is expired. Attempting refresh.")
            if not self.refresh_token:
                logger.warning("JDAuthManager: No refresh token available to refresh expired access token.")
                return None
